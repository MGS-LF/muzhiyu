...
// ============================================================
function drawDialog(ctx, d, gameTime, game) {
...
function drawQuestPanel(ctx, game, px, py, pw, ph, gameTime) {
  ...
  const catColors = {
    '主线': '#ffd870', '可选': '#b8c7ff', '支线': '#80d8ff',
    '收集': '#a0ffa0', '倾向': '#ffa0d0', '刻字': '#d0a0ff',
    '余烬': '#ffb070', '规则': '#ff80ff',
  };
  let yy = py + 58;
  let curCat = '';
  ...
  for (const q of quests) {
    ...
    const badgeColor = q.done ? 'rgba(100,190,120,0.72)' : (q.optional ? 'rgba(120,145,230,0.72)' : 'rgba(255,210,100,0.75)');
    ...
    ctx.fillStyle = q.done ? 'rgba(155,215,155,0.72)' : (q.optional ? 'rgba(210,215,245,0.8)' : 'rgba(238,225,190,0.9)');
  ...
  ctx.fillText('主线是通关所需；可选目标提供复活点、补给或剧情记录。按 J / Esc 关闭', px + pw / 2, py + ph - 16);
  ctx.textAlign = 'left';
}
...
function drawMapPanel(ctx, game, px, py, pw, ph, gameTime) {
  ...
  const links = [
    ...
    ['network_nexus','memory_abyss'], ['memory_abyss','data_center'], ['ruined_library','lost_village'],
  ];
...
function drawInventoryPanel(ctx, game, px, py, pw, ph, gameTime) {
  ...
  const barX = px + 32, barY = py + 54, barW = pw - 64, barH = 12;
  ...
  if (!data.items.length) {
    ...
    const selected = Math.max(0, data.items.findIndex(it => it.selected));
