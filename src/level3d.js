...
const ORIGIN_X = (-MAP_W * CELL) / 2,
  ORIGIN_Z = (-MAP_H * CELL) / 2;
const cellToWorld = (gx, gy) => ({
  x: ORIGIN_X + (gx + 0.5) * CELL,
  z: ORIGIN_Z + (gy + 0.5) * CELL,
});
...
export class Level3D {
  ...
  _buildLevel() {
    ...
    for (let i = 0; i <= MAP_W; i++) {
      ...
      const g = new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(x, 0.02, ORIGIN_Z),
        new THREE.Vector3(x, 0.02, ORIGIN_Z + MAP_H * CELL),
      ]);
    ...
    for (let i = 0; i <= MAP_H; i++) {
      ...
      const g = new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(ORIGIN_X, 0.02, z),
        new THREE.Vector3(ORIGIN_X + MAP_W * CELL, 0.02, z),
      ]);
      this.scene.add(new THREE.Line(g, seamMat));
    }
    ...
    for (let gy = 0; gy < MAP_H; gy++) {
      for (let gx = 0; gx < MAP_W; gx++) {
        ...
        if (ch === '#') {
          ...
          trim.position.set(w.x, 0.09, w.z);
          this.scene.add(trim);
          // 墙顶线条
          const trim2 = new THREE.Mesh(trimGeo, wallTrimMat);
          trim2.position.set(w.x, 3.31, w.z);
          this.scene.add(trim2);
        }
      }
    }
    ...
    this.exitMesh.add(portal);
    // 内层光柱
    const inner = new THREE.Mesh(
      new THREE.CylinderGeometry(0.4, 0.4, 3.2, 16, 1, true),
    ...
    this.exitMesh.add(ring);
    this.exitRing = ring;
    const pl = new THREE.PointLight(0x66ddff, 3.0, 10);
    pl.position.y = 1.6;
  ...
  _addWallLamp(w) {
    ...
    const arm = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x2a2830, metalness: 0.6 })
    );
    arm.position.z = -0.15;
    grp.add(arm);
    // 灯罩
    const shade = new THREE.Mesh(
      new THREE.ConeGeometry(0.22, 0.25, 12, 1, true),
  ...
  _buildViewModel() {
    ...
    arm.position.set(0.28, -0.26, -0.5);
    grp.add(arm);
    // 枪身
    const gun = new THREE.Mesh(
      new THREE.BoxGeometry(0.1, 0.14, 0.5),
      new THREE.MeshStandardMaterial({ color: 0x445566, metalness: 0.7, roughness: 0.3 })
    );
    gun.position.set(0.28, -0.22, -0.7);
    grp.add(gun);
    // 枪管
    const barrel = new THREE.Mesh(
      new THREE.CylinderGeometry(0.025, 0.025, 0.35, 8),
      new THREE.MeshStandardMaterial({ color: 0x2a2a30, metalness: 0.8 })
    );
    ...
    flash.position.set(0.28, -0.22, -1.15);
    grp.add(flash);
  ...
  _spawnGun(w) {
    ...
    g.add(halo);
    g.position.set(w.x, 0.6, w.z);
    g.userData = { type: 'gun', taken: false, baseY: 0.6 };
    this.scene.add(g);
    this.pickups.push(g);
  }
  ...
  _spawnPickup(w, type) {
    ...
    // 光环
    const ring = new THREE.Mesh(
      new THREE.RingGeometry(0.35, 0.5, 18),
      new THREE.MeshBasicMaterial({
        color,
        transparent: true,
        opacity: 0.5,
    ...
    grp.add(ring);
    grp.position.set(w.x, 0.5, w.z);
    grp.userData = { type, taken: false, baseY: 0.5 };
    this.scene.add(grp);
    this.pickups.push(grp);
  }
  ...
  _spawnKeyItem(w) {
    ...
    grp.add(halo);
    const pl = new THREE.PointLight(0xffcc44, 1.5, 4);
    pl.position.y = 0.3;
    grp.add(pl);
    grp.position.set(w.x, 0.7, w.z);
    grp.userData = { type: 'key', taken: false, baseY: 0.7 };
    this.scene.add(grp);
    this.pickups.push(grp);
  }
  ...
  _spawnEnemy(w) {
    ...
    e1.position.set(-0.16, 0.06, 0.4);
    grp.add(e1);
    const e2 = new THREE.Mesh(new THREE.SphereGeometry(0.09, 8, 8), eyeMat);
    e2.position.set(0.16, 0.06, 0.4);
    grp.add(e2);
    // 眼内高光
    const hl = new THREE.MeshBasicMaterial({ color: 0x88ff88 });
    const h1 = new THREE.Mesh(new THREE.SphereGeometry(0.03, 6, 6), hl);
    h1.position.set(-0.14, 0.08, 0.46);
    grp.add(h1);
    const h2 = new THREE.Mesh(new THREE.SphereGeometry(0.03, 6, 6), hl);
    h2.position.set(0.18, 0.08, 0.46);
    grp.add(h2);
    const light = new THREE.PointLight(0x55dd55, 1.4, 4.5);
    grp.add(light);
  ...
  _updateHUD() {
    ...
    this.hud.querySelector('#l3d_ammo').textContent = this.gun.have
      ? `弹药 ${this.gun.ammo}`
      : '无武器';
  ...
  update(dt, input) {
    ...
    for (const e of this.enemies) {
      ...
      // 旋转内核
      if (e.core) {
        e.core.rotation.y += dt * 0.003;
        e.core.rotation.x += dt * 0.002;
      }
    ...
    for (const p of this.pickups) {
      ...
      if (d < 0.8) {
        ...
        if (p.userData.type === 'gun') {
          this.gun.have = true;
          this.gun.ammo = 15;
          this._hintText = '捡到「刻字枪」！左键射击';
          if (this.gunViewModel) this.gunViewModel.visible = true;
        } else if (p.userData.type === 'ammo') {
          this.gun.ammo = Math.min(this.gun.maxAmmo, this.gun.ammo + 10);
          this._hintText = '弹药 +10';
        } else if (p.userData.type === 'health') {
          this.hp = Math.min(this.maxHp, this.hp + 25);
          this._hintText = '理性 +25';
        } else if (p.userData.type === 'key') {
          this.hasKey = true;
          this._hintText = '获得「裂隙之钥」！现在可以穿过出口了。';
        }
      }
    }
    ...
    if (dex < 1.2 && !this.exitReached) {
      if (!this.hasKey) {
        if (!this._keyWarned) {
          ...
          this._hintText = '出口被维度锁封住。需要找到「裂隙之钥」才能通过。';
        }
      } else {
  ...
  _shoot() {
    ...
    const lineGeo = new THREE.BufferGeometry().setFromPoints([
      origin.clone().add(dir.clone().normalize().multiplyScalar(0.5)),
      end,
    ]);
    const line = new THREE.Line(
      lineGeo,
      new THREE.LineBasicMaterial({ color: 0xffeeaa, transparent: true, opacity: 0.5 })
    );
    this.scene.add(line);
    setTimeout(() => {
      if (!this.disposed) this.scene.remove(line);
    }, 60);
  }

  _findEnemyByMesh(mesh) {
    for (const e of this.enemies) {
      if (e.alive && (e.mesh === mesh || e.body === mesh || (e.core && e.core === mesh))) return e;
    }
    return null;
  }
